...
class ${type.name}${ ' ({})'.format(pythonFuncs.getExtendsType(type, modelTypes, baseModelDomain)) if type.extendsType is not None else ''}:
    ...
    def dictToObject(cls, dict):
        ...
        obj.${property.name} = dict.get('${property.name}', ${property.type.default})
                % else:
        ...
        obj.${property.name} = ${pythonFuncs.getTypeWithPackage(property.type, modelTypes, baseModelDomain)}.dictToObject(dict.get('${property.name}', ${property.type.default}))
                % else:
